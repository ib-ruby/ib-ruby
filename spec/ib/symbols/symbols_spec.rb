...
describe IB::Symbols do
  ...
  it 'raises an error if contract symbol is not defined' do
    lambda{wfc = IB::Symbols::Stocks[:xyz]}.should raise_error
    lambda{wfc = IB::Symbols::Options[:xy20z]}.should raise_error
    lambda{fx = IB::Symbols::Forex[:abcdef]}.should raise_error
    lambda{fx = IB::Symbols::Futures[:abc]}.should raise_error
  end

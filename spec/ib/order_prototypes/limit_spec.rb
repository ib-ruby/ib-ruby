...
def place_the_limit_order  # returns the used order-price
		...
		order = IB::Limit.order price: the_order_price , action: :buy, size: 100, account: ACCOUNT
		puts "the order price: #{the_order_price}"
		ib.place_order order, contract      
		ib.wait_for :OpenOrder, 3

		the_order_price #  return_value
...
RSpec.describe IB::Limit do
	before(:all) do
		...
		@the_order_price =  place_the_limit_order

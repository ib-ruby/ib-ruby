...
public abstract class ExampleBase extends Thread implements EWrapper {
    ...
    /**Create Contract method with different signature to allow for indices
     * as well as stock symbols
     * @param symbol
     * @return
     */
    protected Contract createContract(String symbol) {
        if(symbol.equals("INDU")){ //Dow Jones
            return createContract(symbol, "IND", "NYSE", "USD", null, null, 0.0);
        } else if(symbol.equals("COMP")){// Nasdaq
            return createContract(symbol, "IND", "NASDAQ", "USD", null, null, 0.0);
        } else if(symbol.equals("SPX")){ // S&P 500
            return createContract(symbol, "IND", "CBOE", "USD", null, null, 0.0);
        } else { // Assume symbol is ordinary stock
            return createContract(symbol, "STK", "SMART", "USD", null, null, 0.0);
        }
